...
Registration is a term for the alignment of coordinate systems and can be used for diagnostics as well as for therapy: in image-to-image registration141, a typical application is the registration of functional imaging, e.g. positron emission tomography (PET), to anatomical imaging, e.g. computed tomography (CT). As a result, information from corresponding regions is displayed at the same site to enhance the diagnostic value of the image data with additional information84. Image data of the same Methods of tracking
In frameless stereotaxy, after registration of patient and image, any position of a pointing device in the patient's body is displayed accordingly in the image data. Usually, the tip is projected as crosshairs in the corresponding axial, coronal and sagittal slices of the image data, or in a 3D visualization. If the patient is immobilized, as in frame-based stereotaxy, tracking of the pointing device is sufficient, but if movement of the patient is desired, the position of the head has to be
